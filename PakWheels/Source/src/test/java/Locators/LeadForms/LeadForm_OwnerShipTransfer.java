...
public class LeadForm_OwnerShipTransfer extends Leadforms
{
	...
	public LeadForm_OwnerShipTransfer(AppiumDriver<MobileElement> driver, ExtentTest test, WebDriverWait wait, JSONObject Object) 
	{
	...
	public void NavigateToOwnerShipTransferPage() throws Exception{
		DataPicker("Verify that user can tap on Auto Services button to open Drop-Down", "Auto Services", 0);    
		DataPicker("Verify that user can navigate to Ownership Transfer page by cliking on Ownership Transfer button", "Ownership Transfer", 0);
		test.log(Status.INFO, "User is navigated to Ownership Transfer page");	
	}
	
	public void CickOnOwnerShipTransferButon() throws Exception
	{
		DataPicker("Verify that user can click on <b>Transfer ownership to my name button</b>", "Transfer ownership to my name", 2);
		test.log(Status.INFO, "User is navigated to Ownership transfer form");	
	...
	public void CheckThankyouScreen() throws InterruptedException
	{
		...
		if (ActualText.equalsIgnoreCase("Thank you for choosing PakWheels")) 
		{
			...
			test.log(Status.PASS, "Car Transform request has been submitted successfully");
		...
		{
			test.log(Status.FAIL, "Car Transfer request has been FAILED");
		}
	}

	public void VerifyTransferLandingPage() throws Exception {
		...
		if(Base.Testing.equalsIgnoreCase("System"))
		{
			...
			if(h1.equalsIgnoreCase("PakWheels Car Ownership Transfer"))
			{
			...
			if(button.equalsIgnoreCase("Transfer ownership to my name"))
			{
		        ...
		        {
		        	if(text.equals("How car ownership transfer works?"))
		        	{
		        		test.log(Status.PASS,"Verified that "+text+" section is showing");
		        	}
		        
		        	if(text.equals("Why choose PakWheels Car Ownership Transfer?"))
		        	{
		        		test.log(Status.PASS,"Verified that "+text+" section is showing");
		        	}

		        	if(text.equals("Car Ownership Transfer FAQs"))
		        	{
		        		test.log(Status.PASS,"Verified that "+text+" section is showing");
		        	}
		        }
		     ...
		     // Scroll to the end of the screen
		    	...
		    	if(driver.findElement(By.id("com.pakwheels.staging:id/btn_new_product_page_bottom_sticky")).isDisplayed())
	        	{
					...
					test.log(Status.PASS, "Verify that user can click on Get My Car Transfered on button from Sticky button on Car Ownership Transfer landing page.");
